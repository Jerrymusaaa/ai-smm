...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI insight banner */}
      <div className="rounded-2xl border border-[#A855F7]/20 p-4 flex items-start gap-4"
        style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.08), rgba(0,102,255,0.06))' }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-white/50 leading-relaxed">
            Your best month yet — overall engagement is up <span className="text-white font-medium">28%</span> vs March.
            TikTok drove the most growth with <span className="text-white font-medium">+24% followers</span>.
            Your top content format is short-form video at <span className="text-white font-medium">8.4% engagement rate</span>.
            Posting at 6–8 PM EST consistently outperforms other windows by <span className="text-white font-medium">2.1x</span>.
          </p>
        </div>
        <button className="text-xs text-white/30 hover:text-white transition-colors flex-shrink-0">
          Full report →
        </button>
      </div>
      ...
      {/* Platform comparison + audience */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <PlatformComparison />
        <AudienceDemographics />
      </div>

      {/* Content breakdown + report generator */}
