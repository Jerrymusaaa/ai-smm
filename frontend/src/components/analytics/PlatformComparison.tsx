...
const PLATFORMS = [
  { id: 'instagram', label: 'Instagram', color: '#E1306C', initial: 'IG',
    followers: 12441, impressions: 480000, engagement: 4.8, clicks: 5200, growth: 18, posts: 142 },
  { id: 'tiktok', label: 'TikTok', color: '#FF0050', initial: 'TT',
    followers: 8204, impressions: 620000, engagement: 7.2, clicks: 3800, growth: 24, posts: 89 },
  { id: 'linkedin', label: 'LinkedIn', color: '#0A66C2', initial: 'IN',
    followers: 3109, impressions: 94000, engagement: 3.1, clicks: 2100, growth: 12, posts: 56 },
  { id: 'twitter', label: 'X / Twitter', color: '#1DA1F2', initial: 'X',
    followers: 5677, impressions: 210000, engagement: 2.4, clicks: 1748, growth: 6, posts: 210 },
];
...
const METRIC_LABELS: Record<string, string> = {
  followers: 'Followers', impressions: 'Impressions', engagement: 'Eng. rate',
  clicks: 'Link clicks', growth: 'Growth %', posts: 'Posts',
};
    ...
    <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        {sorted.map((p, i) => {
          ...
          const formatted = metric === 'engagement' ? `${val}%`
            : metric === 'growth' ? `+${val}%`
      ...
      <div className="px-5 pb-5 border-t border-white/[0.06] pt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1.5">
            {PLATFORMS.map(p => (
              <button key={p.id}
                onClick={() => setActiveRadar(prev =>
                  prev.includes(p.id) ? prev.filter(x => x !== p.id) : [...prev, p.id]
                )}
                className="w-6 h-6 rounded-md flex items-center justify-center text-[9px] font-bold text-white transition-all"
                style={{
                  background: activeRadar.includes(p.id) ? `${p.color}30` : 'rgba(255,255,255,0.05)',
                  border: `1px solid ${activeRadar.includes(p.id) ? p.color : 'rgba(255,255,255,0.1)'}`,
                  opacity: activeRadar.includes(p.id) ? 1 : 0.4,
                }}>
                {p.initial}
              </button>
            ))}
