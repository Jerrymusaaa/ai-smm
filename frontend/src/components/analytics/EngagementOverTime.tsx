    ...
    <div className="glass rounded-2xl p-5 border border-white/[0.06]">
            ...
            {METRICS.map(m => (
              <button key={m.key} onClick={() => toggle(m.key)}
                ...
                style={active.includes(m.key)
                  ? { background: `${m.color}20`, borderColor: `${m.color}40`, color: m.color }
                  : { borderColor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.3)' }}>
                <div className="w-2 h-2 rounded-full"
                  style={{ background: active.includes(m.key) ? m.color : 'rgba(255,255,255,0.2)' }} />
                {m.label}
              </button>
            ))}
          </div>
      ...
      <div className="h-60">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={DATA_30D} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            <defs>
              {METRICS.map(m => (
            ...
            {METRICS.map(m => active.includes(m.key) && (
              <Area key={m.key} type="monotone" dataKey={m.key} name={m.label}
                stroke={m.color} strokeWidth={2} fill={`url(#ag-${m.key})`}
