...
function VariantCard({ variant, onRegenerate, onSchedule }: {
  variant: Variant;
  onRegenerate: (id: string) => void;
  onSchedule: (v: Variant) => void;
}) {
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.04]">
        <div
          ...
          style={{ background: `${variant.platformColor}25`, border: `1px solid ${variant.platformColor}35` }}
        >
          {variant.platformInitial}
        </div>
      ...
      {expanded && (
        <>
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {variant.hashtags.map(tag => (
                <span key={tag}
                  className="text-xs px-2 py-0.5 rounded-full cursor-pointer transition-colors hover:opacity-80"
                  style={{ background: `${variant.platformColor}15`, color: variant.platformColor }}>
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-t border-white/[0.04]">
            ...
            <button onClick={() => onRegenerate(variant.id)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white/50 hover:text-white hover:bg-white/[0.06] transition-all">
        ...
        {[1, 2, 3].map(i => (
          <div key={i} className="glass rounded-2xl border border-white/[0.06] p-4 animate-pulse">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="h-4 bg-white/[0.08] rounded-lg w-24" />
              <div className="ml-auto h-4 bg-white/[0.08] rounded-full w-12" />
            </div>
            ...
            <div className="flex gap-2 mt-4">
              {[1,2,3].map(j => <div key={j} className="h-6 bg-white/[0.06] rounded-full w-16" />)}
            </div>
          </div>
        ))}
    ...
    return (
      <div className="glass rounded-2xl border border-white/[0.06] p-10 text-center">
        ...
        <p className="text-white/25 text-xs">
          Describe your content above and click Generate to create AI-optimized captions for each platform.
        </p>
      </div>
    );
  }
