    ...
    <div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active
                  ? 'text-white'
                  : 'border-white/[0.08] bg-transparent text-white/40 hover:text-white/70 hover:border-white/20'
              )}
              style={active ? {
            ...
            >
              ...
              {p.label}
              {active && (
                <span className="text-[10px] opacity-60">{p.maxChars >= 1000 ? `${(p.maxChars/1000).toFixed(0)}K` : p.maxChars}</span>
              )}
            </button>
          );
        })}
      ...
      {selected.length > 0 && (
        <p className="text-xs text-white/30 mt-2">
          {selected.length} platform{selected.length > 1 ? 's' : ''} selected · AI will optimize each caption separately
        </p>
      )}
    </div>
  );
