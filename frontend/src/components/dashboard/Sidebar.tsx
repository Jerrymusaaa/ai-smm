...
const NAV_ITEMS = [
  ...
  { label: 'AI Assistant', href: '/dashboard/assistant', icon: MessageSquare, badge: 'AI' },
...
export function Sidebar({ open, collapsed, onClose, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside className={cn(
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <button onClick={onToggleCollapse}
            className="hidden lg:flex p-1.5 rounded-lg text-white/30 hover:text-white hover:bg-white/5 transition-all">
      ...
      {collapsed && (
        <button onClick={onToggleCollapse}
          className="hidden lg:flex mx-auto mt-3 p-1.5 rounded-lg text-white/30 hover:text-white hover:bg-white/5 transition-all">
          ...
          return (
            <Link key={item.label} href={item.href}
              ...
              {!collapsed && item.badge && (
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#0066FF]/20 text-[#0066FF]">
                  {item.badge}
                </span>
              )}
          ...
          <div className="pt-4 pb-2">
            ...
            <div className="space-y-0.5">
              {CONNECTED_PLATFORMS.map(p => (
                <div key={p.name}
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/[0.06] transition-all cursor-pointer">
      ...
      <div className="px-2 py-3 border-t border-white/[0.06] space-y-0.5">
        ...
        {!collapsed ? (
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/[0.06] transition-all cursor-pointer mt-1">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#0066FF] to-[#00D4AA] flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
              JD
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium text-white truncate">Jerry D.</div>
          ...
          <div className="flex justify-center mt-1">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#0066FF] to-[#00D4AA] flex items-center justify-center text-xs font-bold text-white cursor-pointer">
              JD
            </div>
          </div>
        )}
