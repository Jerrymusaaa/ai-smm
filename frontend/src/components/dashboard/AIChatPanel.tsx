...
export function AIChatPanel() {
  ...
  const sendMessage = async (text: string) => {
    if (!text.trim()) return;
    const userMsg: Message = { id: Date.now(), role: 'user', text, time: 'Just now' };
    ...
    const responseKey = text.toLowerCase().includes('caption') ? 'captions'
      ...
      <div className="px-4 pb-4 pt-2 flex-shrink-0 border-t border-white/[0.04]">
        <div className="flex items-end gap-2 bg-white/[0.05] rounded-2xl border border-white/10 px-4 py-3 focus-within:border-[#C9A84C]/40 transition-colors">
          <textarea
            ...
            rows={1}
            className="flex-1 bg-transparent text-sm text-white placeholder-white/25 outline-none resize-none leading-relaxed max-h-28"
