    ...
    <header className="h-16 flex items-center gap-4 px-4 sm:px-6 border-b border-white/[0.06] flex-shrink-0"
      ...
      <div className="flex-1 max-w-md mx-auto hidden md:block">
        <div className={`flex items-center gap-2.5 px-3.5 py-2 rounded-xl border transition-all ${
          searchOpen ? 'border-[#0066FF]/40 bg-white/[0.07]' : 'border-white/[0.06] bg-white/[0.03]'
        }`}>
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div className="relative">
          ...
          {userMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-white/10 shadow-2xl z-50 overflow-hidden"
              style={{ background: '#0D1525' }}>
              <div className="px-4 py-3 border-b border-white/[0.06]">
                <p className="text-sm font-medium text-white">{user?.name || 'User'}</p>
                <p className="text-xs text-white/40 truncate">{user?.email}</p>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#0066FF]/20 text-[#0066FF] font-medium mt-1 inline-block">
                  {user?.plan || 'FREE'} Plan
                </span>
              </div>
              <div className="p-2">
