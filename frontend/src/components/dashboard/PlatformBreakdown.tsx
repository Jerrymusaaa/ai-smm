...
const PLATFORMS = [
  { name: 'Instagram', initial: 'IG', color: '#E1306C', followers: '12,441', engagement: '4.8%', posts: 142, growth: '+18%', trend: 'up' },
  { name: 'TikTok', initial: 'TT', color: '#FF0050', followers: '8,204', engagement: '7.2%', posts: 89, growth: '+24%', trend: 'up' },
  { name: 'LinkedIn', initial: 'IN', color: '#0A66C2', followers: '3,109', engagement: '3.1%', posts: 56, growth: '+12%', trend: 'up' },
  { name: 'X / Twitter', initial: 'X', color: '#1DA1F2', followers: '5,677', engagement: '2.4%', posts: 210, growth: '+6%', trend: 'up' },
];
      ...
      <div className="px-5 py-3 border-b border-white/[0.04]">
        <div className="flex rounded-full overflow-hidden h-2 gap-px">
          {PLATFORMS.map(p => {
            ...
            return (
              <div key={p.name} style={{ width: `${pct}%`, background: p.color }}
                className="transition-all" title={`${p.name}: ${pct.toFixed(1)}%`} />
            );
          })}
        </div>
      </div>
