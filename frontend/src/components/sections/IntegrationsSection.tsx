    ...
    <section id="integrations" className="py-24 relative overflow-hidden">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {[row1, row2].map((row, rowIdx) => (
            <div key={rowIdx} className="flex flex-wrap justify-center gap-3">
              {row.map((platform) => (
                <div
                  key={platform.name}
                  className="platform-badge flex items-center gap-2.5 px-4 py-2.5 glass rounded-xl border border-white/[0.06] hover:border-white/20 cursor-pointer"
                >
                  <div
                    className="w-6 h-6 rounded-md flex-shrink-0"
                    style={{ background: `${platform.color}25`, border: `1px solid ${platform.color}40` }}
                  />
                  <span className="text-sm text-white/70 whitespace-nowrap">{platform.name}</span>
                  <span className="text-xs px-1.5 py-0.5 rounded-full text-white/30 bg-white/[0.04]">
                    {platform.category}
                  </span>
                </div>
              ))}
        ...
        <p className="text-center text-white/30 text-sm mt-6">
          All 23 platforms included in every plan · OAuth-secured connections
        </p>
      </div>
    </section>
  );
